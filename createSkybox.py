...
def main(image_name):
    imgIn = Image.open(image_name)
    inSize = imgIn.size

    map_x_32, map_y_32 = generate_mapping_data(int(inSize[0]))
    cubemap = cv2.remap(numpy.array(imgIn), map_x_32, map_y_32, cv2.INTER_LINEAR)

    imgOut = Image.fromarray(cubemap)
    cubemap_name = image_name.split('.')[0] + "_cubemap.png"
    imgOut.save(cubemap_name)

    return str(cubemap_name)
